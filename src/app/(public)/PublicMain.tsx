...
type Heading = { title: React.ReactNode; subtitle: string };

function useRegistryHeading(pathname: string): Heading | null {
  const tPublic = useTranslations("public");
  const tNav = useTranslations("nav");
  const comingSoon = tPublic("comingSoon");
  const comingSoonSubtitle = tPublic("registry.section", {
    status: comingSoon,
  });

  function navTitle(key: "creators" | "generators" | "influencers") {
    return (
      <span className={publicMainClasses.headingLabel}>
        {tNav(key).toUpperCase()}
        <span className={publicMainClasses.headingApostrophe}>’</span>
      </span>
    );
  }
...
export function PublicMain({ children }: { children: React.ReactNode }) {
  ...
  const showSkeleton =
    ...
    isRegistryTopPage(activePathname);
